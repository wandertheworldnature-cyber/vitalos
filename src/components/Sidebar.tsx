...
export default function Sidebar() {
  ...
  useEffect(() => {
    if (user?.email) { checkIfDoctor(); loadUnreadAlerts() }
  }, [user?.email])

  async function checkIfDoctor() {
    const { data } = await supabase.from('doctors').select('id').eq('doctor_email', user?.email).single()
    setIsDoctor(!!data)
  }
  ...
  const initials = user?.full_name ? user.full_name.split(' ').map((n: string) => n[0]).join('').slice(0,2).toUpperCase() : user?.email?.slice(0,2).toUpperCase() || 'U'
  const planColors: Record<string, string> = { basic: 'bg-gray-100 text-gray-600', pro: 'bg-blue-100 text-blue-700', premium: 'bg-amber-100 text-amber-700' }
    ...
    <aside className="w-56 flex flex-col shrink-0 h-screen sticky top-0"
      ...
      <div className="px-4 py-4 border-b border-emerald-100">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-md" style={{ background: 'linear-gradient(135deg,#0f6e56,#1d9e75)' }}>
        ...
        {navItems.map(({ to, icon: Icon, label, badge, badgeColor }) => (
          <NavLink key={to} to={to}
            ...
            {to === '/insights' && unreadAlerts > 0 && (
              <span className="w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[9px] text-white font-bold shrink-0">{unreadAlerts}</span>
            )}
          </NavLink>
        ))}
      </nav>

      <div className="mx-3 mb-2 p-3 rounded-xl border border-emerald-100" style={{ background: 'linear-gradient(135deg,#ecfdf5,#d1fae5)' }}>
      ...
      <div className="p-3 border-t border-emerald-100">
        <div className="flex items-center gap-2.5 mb-2.5">
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm shrink-0" style={{ background: 'linear-gradient(135deg,#0f6e56,#1d9e75)' }}>
            {initials}
          </div>
          <div className="min-w-0">
            <div className="text-xs font-semibold text-gray-900 truncate">{user?.full_name || user?.email?.split('@')[0] || 'User'}</div>
            <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-semibold capitalize ${planColors[user?.plan||'basic']}`}>{user?.plan||'basic'} plan</span>
          </div>
        </div>
