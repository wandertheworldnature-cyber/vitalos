...
import { RefreshCw, ArrowRight, Sparkles, TrendingUp, AlertTriangle,
         CheckCircle, Upload, Plus, Activity } from 'lucide-react'
...
export default function Dashboard() {
  ...
  function calcStatus(v: number, min?: number, max?: number): MetricCardType['status'] {
  ...
  async function handleRefreshInsights() {
    ...
    try {
      ...
      toast.error('Add VITE_GROQ_API_KEY to .env for AI insights')
    ...
    <div className="p-6 space-y-6 max-w-6xl">
      ...
      <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            {critCount > 0 && (
              <span className="flex items-center gap-1 text-xs text-red-600 font-medium">
                <AlertTriangle size={13} /> {critCount} critical
              </span>
            )}
            {warnCount > 0 && (
              <span className="flex items-center gap-1 text-xs text-amber-600 font-medium">
                <AlertTriangle size={13} /> {warnCount} warning{warnCount > 1 ? 's' : ''}
              </span>
            )}
            {!loading && hasData && critCount === 0 && warnCount === 0 && (
              <span className="flex items-center gap-1 text-xs text-emerald-600 font-medium">
                <CheckCircle size={13} /> All metrics healthy
              </span>
            )}
            {!loading && !hasData && (
              <span className="text-xs text-gray-400">Upload your first lab report to see insights</span>
            )}
          </div>
        </div>
        ...
        {score ? (
              ...
              <div className="flex items-end gap-1.5 h-12">
                {Object.entries(score.breakdown).map(([key, val]) => (
                  <div key={key} className="flex flex-col items-center gap-1">
                    <div className="w-4 rounded-t-sm"
          ...
          <div className="card py-4 px-6 text-center"
            ...
            <button onClick={() => navigate('/reports')}
              className="mt-2 text-[11px] text-teal-600 font-semibold hover:underline">
              Upload now →
            </button>
          </div>
        )}
      ...
      <div>
        ...
        {!loading && metrics.length === 0 ? (
          <div className="card border-dashed border-2 border-gray-200 text-center py-10">
            ...
            <p className="text-xs text-gray-400 mt-1 mb-4">Upload a lab report or add readings manually</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {loading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="rounded-2xl bg-gray-100 h-32 animate-pulse" />
                ))
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          {insights.length === 0 && !loading ? (
            <div className="card border-dashed border-2 border-purple-100 text-center py-8">
              ...
              <p className="text-xs text-gray-400 mt-1 mb-4">
                Upload a lab report — AI will analyze your data and generate personalized insights
              </p>
        ...
        <div className="space-y-4">
          <div className="card border-dashed border-2 border-teal-200"
          ...
          <div className="card">
            ...
            <div className="space-y-1.5">
              {[
                ...
                { e:'📈', l:'View trends', a: () => navigate('/trends'), c:'hover:bg-emerald-50 hover:text-emerald-700' },
              ].map(a => (
