...
import { Star, Video, Clock, Check, X, Calendar, Brain, Stethoscope, AlertTriangle } from 'lucide-react'
...
// AI suggestion engine — matches insights to specialties
function getAISuggestions(insights: Array<{ severity: string; title: string; description: string; related_metrics?: string[] }>): Array<{ specialty: string; reason: string; urgency: 'urgent'|'recommended'|'routine' }> {
  ...
  if (all.some(t => t.includes('glucose') || t.includes('hba1c') || t.includes('diabet') || t.includes('insulin')))
    suggestions.push({ specialty: 'Endocrinologist', reason: 'High glucose or HbA1c pattern detected', urgency: 'urgent' })

  if (all.some(t => t.includes('cholesterol') || t.includes('ldl') || t.includes('cardiovascular') || t.includes('triglyceride') || t.includes('cardiac')))
    suggestions.push({ specialty: 'Cardiologist', reason: 'Lipid profile concerns — cardiovascular risk', urgency: 'recommended' })

  if (all.some(t => t.includes('tsh') || t.includes('thyroid') || t.includes('vitamin d') || t.includes('b12') || t.includes('iron') || t.includes('hemoglobin')))
    suggestions.push({ specialty: 'Endocrinologist', reason: 'Thyroid, vitamin, or metabolic imbalance', urgency: 'recommended' })

  if (all.some(t => t.includes('creatinine') || t.includes('kidney') || t.includes('uric acid') || t.includes('egfr')))
    suggestions.push({ specialty: 'Nephrologist', reason: 'Kidney function markers need evaluation', urgency: 'urgent' })

  if (all.some(t => t.includes('alt') || t.includes('ast') || t.includes('liver') || t.includes('bilirubin')))
    suggestions.push({ specialty: 'Gastroenterologist', reason: 'Liver enzyme elevation detected', urgency: 'recommended' })

  if (all.some(t => t.includes('anemia') || t.includes('hemoglobin') || t.includes('platelet') || t.includes('wbc')))
    suggestions.push({ specialty: 'General Physician', reason: 'Blood count abnormality needs review', urgency: 'routine' })

  // Default preventive
  if (suggestions.length === 0)
    suggestions.push({ specialty: 'Preventive Medicine', reason: 'Annual preventive health check-up', urgency: 'routine' })

  // Deduplicate by specialty
  return suggestions.filter((s, i, arr) => arr.findIndex(x => x.specialty === s.specialty) === i).slice(0, 3)
}
...
export default function DoctorsPage() {
  ...
  const [availableReports, setAvailableReports] = useState<Array<{id:string;file_name:string;lab_name:string|null;created_at:string}>>([])
  ...
  async function loadAISuggestions() {
    if (!user) return
    const { data } = await supabase.from('ai_insights').select('severity,title,description,related_metrics')
      .eq('user_id', user.id).order('generated_at', { ascending: false }).limit(10)
    if (data && data.length > 0) setAiSuggestions(getAISuggestions(data as Array<{ severity: string; title: string; description: string; related_metrics?: string[] }>))
  }

  async function loadReports() {
    if (!user) return
    const { data } = await supabase.from('health_reports').select('id,file_name,lab_name,created_at')
      .eq('user_id', user.id).eq('ocr_status', 'done').order('created_at', { ascending: false }).limit(10)
    setAvailableReports((data || []) as Array<{id:string;file_name:string;lab_name:string|null;created_at:string}>)
  }
  ...
  async function handleBook() {
    ...
    try {
      ...
      const { data: apptData, error } = await supabase.from('appointments').insert({
        user_id: user.id, doctor_id: selected.id,
        slot_date: selectedSlot.slot_date, slot_time: selectedSlot.slot_time,
        notes, status: 'scheduled', meeting_link: meetLink,
      ...
      try {
        await supabase.functions.invoke('send-appointment-email', { body: { appointmentId: apptData.id, type: 'confirmed' } })
      ...
      loadAppointments(); setTab('appointments')
    } catch (err) {
  ...
  async function cancelAppointment(apptId: string, slotDate: string, slotTime: string, doctorId: string) {
    ...
    await supabase.from('doctor_slots').update({ is_booked: false }).eq('doctor_id', doctorId).eq('slot_date', slotDate).eq('slot_time', slotTime)
    try { await supabase.functions.invoke('send-appointment-email', { body: { appointmentId: apptId, type: 'cancelled' } }) } catch {}
  ...
  const urgencyStyle = { urgent: 'bg-red-50 border-red-200 text-red-700', recommended: 'bg-amber-50 border-amber-200 text-amber-700', routine: 'bg-blue-50 border-blue-200 text-blue-700' }
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {doctors.length === 0 && (
                <div className="col-span-2 card text-center py-12 border-dashed">
                  ...
                  <p className="text-xs text-gray-300">Admin → Doctors → Add doctor</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="card border-teal-200" style={{ background:'linear-gradient(135deg,#f0fdf8,#ecfdf5)' }}>
              ...
              {slotsLoading ? (
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-center">
                  ...
                  <p className="text-xs text-amber-600 mt-1">Doctor hasn't set availability yet. Contact admin.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {availableReports.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-gray-700 mb-2">Attach lab reports for doctor to review</p>
                      <div className="flex flex-wrap gap-2">
                      ...
                      {attachReports.length > 0 && (
                        <p className="text-[10px] text-teal-600 mt-1">✓ {attachReports.length} report(s) will be shared with the doctor</p>
                      )}
                    </div>
                  )}

                  <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-700">
                    📧 Confirmation sent to <strong>{user?.email}</strong> from VitalOS on behalf of {selected.name}
                  </div>
